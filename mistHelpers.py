...
class MistHelpers:
    ...
    def get_aps(self, site):
        devices = None
        devices = self.api.get("sites/{0}/devices?type=ap".format(site))
        return devices
    def get_switches(self, site):
        devices = None
        devices = self.api.get("sites/{0}/devices?type=switch".format(site))
        return devices

    def get_gateways(self, site):
        devices = None
        devices = self.api.get("sites/{0}/devices?type=gateway".format(site))
        return devices

    def get_ap_by_id(self, device_id, site):
        device = None
        device = self.api.get("sites/{0}/stats/devices/{1}".format(site,device_id))
        return device

    def get_ap_by_name(self, device_name, site):
        device = None
        device = self.api.get("sites/{0}/stats/devices?name={1}".format(site,device_name))
        return device

    def get_ap_by_mac2(self, device_mac, site):
        device = None
